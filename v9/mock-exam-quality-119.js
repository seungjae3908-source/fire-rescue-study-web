...
sh=a=>{a=[...a];for(let i=a.length-1;i;i--){const j=Math.floor(Math.random()*(i+1));[a[i],a[j]]=[a[j],a[i]]}return a},
...
recent=(h,n=4)=>new Set((h||[]).slice(-n).flatMap(x=>x.questionIds||[])),
...
function pick(a,n,l,sc,h=[]){
  ...
  while(o.length<n){
    ...
    const q=sh(p).map(q=>[q,rank(q)]).sort((x,y)=>y[1]-x[1])[0][0];add(q)
  }
  return o.length===n?o:[]
}
...
function build({mode='real',level='mid',history=[]}={}){
  const ok=q=>mode==='real'?(q.grade==='A'||q.grade==='B'):V.QuestionQuality119?.isExamStyle?.(q)!==false,all=(V.questions||[]).filter(ok);
