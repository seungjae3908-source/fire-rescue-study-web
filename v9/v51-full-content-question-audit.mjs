...
const verifiedEvidenceStructure=q=>!!q&&Array.isArray(q.choices)&&q.choices.length===4&&new Set(q.choices.map(key)).size===4&&Number.isInteger(q.a)&&q.a>=0&&q.a<4&&compact(q.ex).length>=8&&!!q.source;
...
for(const c of V.curriculum.concepts){
  ...
  for(const q of verified){
    if(!verifiedEvidenceStructure(q))local.push('verified-evidence-structure:'+q.id);
    if(!pageSource.test(String(q.source||''))&&!officialSource.test(String(q.source||'')))local.push('verified-source:'+q.id);
    if(Array.isArray(q.choiceExplanations)&&q.choiceExplanations.length===4&&compact(q.ex)!==compact(q.choiceExplanations[q.a]))local.push('explanation-drift:'+q.id);
