...
try{
 ...
 for(const id of targetIds){
  const result=await page.evaluate(async ({id,extraPages})=>{const V=window.AITUTOR_V9,c=V.curriculum.byId[id],p=V.contentPacks.get(id);const ranges=(c?.sourceRanges||[]).filter(r=>r?.doc&&Number.isFinite(Number(r.from))&&Number.isFinite(Number(r.to)));if(!ranges.length)throw new Error('V45_BATCH8_NO_NUMERIC_SOURCE_RANGE '+id);const rows=[];for(const r of ranges){const entry=await V.SourcePDF.openPdf(r.doc,{timeoutMs:120000}),pdf=entry.pdf;for(let n=1;n<=pdf.numPages;n++){const book=V.SourcePDF.bookPage?.(r.doc,n)||0;const extra=extraPages.includes(book);if((book<Number(r.from)||book>Number(r.to))&&!extra)continue;const pg=await pdf.getPage(n),tc=await pg.getTextContent(),text=(tc.items||[]).map(x=>x.str).join(' ').replace(/\s+/g,' ').trim();rows.push({doc:r.doc,from:r.from,to:r.to,bookPage:book,pdfPage:n,extra,text});}}return{id,title:c?.title||'',packSource:p?.source||'',must:p?.must||[],summary:p?.summary||'',detail:p?.detail||[],ranges,rows};},{id,extraPages:extraReviewPages[id]||[]});
