...
const configured=()=>!!(cfg.supabaseUrl&&cfg.supabaseAnonKey);const iso=x=>x?new Date(x).toISOString():null;const ms=x=>x?Date.parse(x)||0:0;
...
async function fetchRemoteSnapshot(uid){const [profile,progress,answers,wrongs,reviews,notes,sessions,exams,tutor,docs,chunks]=await Promise.all([
  ...
  for(const r of reviews||[])remote.reviewSchedule[r.concept_id]={conceptId:r.concept_id,due:ms(r.due_at),intervalDays:r.interval_days||0,mastery:Number(r.mastery)||0,updatedAt:ms(r.updated_at)};
  remote.notes=(notes||[]).map(r=>({id:r.id,title:r.title,body:r.body,sourceType:r.source_type,private:true,createdAt:ms(r.created_at),updatedAt:ms(r.updated_at)}));
  ...
  const privateDocs=(docs||[]).map(r=>({id:r.id,ownerId:uid,kind:'personal',title:r.title,fileName:r.file_name||'',mime:r.mime_type||'',pageCount:r.page_count||0,sourceHash:r.source_hash||'',private:true,createdAt:ms(r.created_at),updatedAt:ms(r.updated_at),original:null}));
...
async function syncAllInternal(uid){if(!client||!uid)throw Error('NOT_SIGNED_IN');if(V.Store.ownerId!==uid)V.Store.switchOwner(uid);const s=V.Store.state,now=new Date().toISOString(),tasks=[];
  ...
  const notes=(s.notes||[]).map(n=>({id:n.id,user_id:uid,title:n.title||'내 노트',body:n.body||n.text||'',source_type:n.sourceType||'manual',private:true,created_at:iso(n.createdAt||n.updatedAt||Date.now()),updated_at:iso(n.updatedAt||Date.now())}));tasks.push(upsertRows('personal_notes',notes));
...
async function init(){if(readyPromise)return readyPromise;readyPromise=(async()=>{if(!configured())return{client:null,user:null};try{const m=await import('https://esm.sh/@supabase/supabase-js@2');client=m.createClient(cfg.supabaseUrl,cfg.supabaseAnonKey,{auth:{persistSession:true,autoRefreshToken:true,detectSessionInUrl:true}});const {data,error}=await client.auth.getUser();if(error)throw error;user=data?.user||null;client.auth.onAuthStateChange((event,session)=>{const next=session?.user||null;if(event==='SIGNED_OUT'||!next){if(event==='SIGNED_OUT'){user=null;V.Store.switchOwner(V.Store.guestId);emit()}return}user=next;if(event==='SIGNED_IN'||event==='USER_UPDATED')queueAdopt(next).catch(e=>console.warn('member adopt failed',e))});if(user)setTimeout(()=>queueAdopt(user).catch(e=>console.warn('initial member sync failed',e)),0);return{client,user}}catch(e){console.warn('auth init failed',e);client=null;user=null;return{client:null,user:null}}})();return readyPromise}
async function signUp(email,password){await init();if(!client)throw Error('MEMBER_BACKEND_NOT_CONFIGURED');const {data,error}=await client.auth.signUp({email,password});if(error)throw error;if(data?.session&&data?.user)await queueAdopt(data.user);else if(data?.user)throw Error('회원가입 완료 · 이메일 확인 후 로그인해 주세요');return data}
...
async function syncAll(){await init();if(!client||!user)throw Error('NOT_SIGNED_IN');return syncAllInternal(user.id)}
async function pull(){await init();if(!client||!user)throw Error('NOT_SIGNED_IN');return pullRemoteIntoLocal(user.id)}
V.Auth={init,configured,get client(){return client},get user(){return user},get isGuest(){return !user},signUp,signIn,signOut,syncAll,pull,label(){return user?.email||'게스트'},privacy:'personal-data-is-private-by-default',syncPolicy:{remoteFirstOnSignIn:true,originalFilesAutoUpload:false,extractedTextManualCloudSync:true}};
init();
})();
