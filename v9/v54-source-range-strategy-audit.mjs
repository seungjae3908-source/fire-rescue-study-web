...
assert(src.includes("else if(catalog?.proxyPdf)")&&src.includes("official-proxy-range")&&src.includes("disableStream:true"),'proxy textbooks use bounded range-first loading');
assert(src.includes("official-proxy-fallback")&&src.includes("official-proxy-full-cache-fallback"),'mirror and proxy range failures retain full-cache fallback');
