...
const exactAnchorBatch={'F07-C05':284,'F07-C17':288,'F07-C18':284,'F07-C19':284,'F07-C20':302,'F07-C21':287};
...
}),'six sprinkler concepts are exact-page verified from 2026 prevention1 PDF');
ok(scopeVerified.length===35&&scopeVerified.every(id=>/^F0[5-7]-/.test(id)||/^F03-C(09|1[0-6])$/.test(id)),`35 page-anchor-pending fire concepts remain fail-closed; actual=${scopeVerified.length}`);
ok(missing.length===35&&missing.every(id=>scopeVerified.includes(id)),`only the 35 page-anchor-pending concepts are not fully verified; missing=${missing.join(',')||'none'}`);
...
ok(coverage.verified===141&&coverage.pending===35,'release truth advances to 141 page-verified + 35 page-anchor-pending');
