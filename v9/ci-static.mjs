...
ok(appSource.includes('SOURCE_OVERLAY_HISTORY')&&appSource.includes("window.addEventListener('popstate'")&&appSource.includes("e.key==='Escape'"),'official source overlays close safely on browser/system back and Escape');
ok(appSource.includes('data-source-nav="back"')&&appSource.includes('data-source-nav="close"')&&appSource.includes('원문 보기에서 뒤로가기')&&appSource.includes('원문 보기 닫기'),'official source overlays expose explicit accessible back and close controls');
ok(appStyles.includes('v26 official source viewer navigation')&&appStyles.includes('.pdf-modal-head,.source-modal-head{position:sticky')&&appStyles.includes('.pdf-nav-actions'),'official source viewer keeps navigation controls sticky and mobile-readable');
