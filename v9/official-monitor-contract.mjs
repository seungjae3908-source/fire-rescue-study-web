...
assert(gosiFire?.urls?.[0]==='https://gongmuwon.gosi.kr/spcsv/indexMain3.do'&&gosiFire.required===true&&gosiFire.requestTimeoutMs===20000&&gosiFire.attempts===2&&gosiFire.detail===false,'fire recruitment monitoring uses the official National Civil Service Recruitment System as the required source with a bounded latency retry budget and no unsafe aggregate-page detail promotion');
assert(nfsaNotice?.urls?.[0]==='https://www.nfa.go.kr/nfsa/news/0011/job/?pageIdx=1'&&nfsaNotice?.required===false&&nfsaNotice?.detail===false,'NFSA notice monitoring uses the canonical NFA-hosted school route and avoids unsafe aggregate-page detail promotion');
assert(nfsaMaterials?.urls?.[0]==='https://www.nfa.go.kr/nfsa/releaseinformation/archive/materials/'&&nfsaMaterials?.fallbackUrls?.[0]==='https://www.nfa.go.kr/nfsa/releaseinformation/archive/materials/?pageIdx=1'&&nfsaMaterials?.required===false&&nfsaMaterials?.detail===false,'official textbook monitoring uses the canonical NFA materials route and stays list-only to avoid unnecessary detail traffic');
assert(!JSON.stringify(SOURCES).includes('"https://nfsa.go.kr/')&&!JSON.stringify(SOURCES).includes('cherish.nfsa.go.kr'),'V3 removes TLS-bad NFSA hosts and uses the canonical www.nfa.go.kr school paths');
