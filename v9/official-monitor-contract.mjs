...
assert(nfsaNotice?.urls?.[0]==='https://cherish.nfsa.go.kr/nfsa/news/0011/job/?pageIdx=1'&&nfsaNotice?.detail===false,'NFSA notice monitoring uses the exact official service-board path without unsafe aggregate-page detail promotion');
assert(nfsaMaterials?.urls?.[0]==='https://cherish.nfsa.go.kr/nfsa/releaseinformation/archive/materials/'&&nfsaMaterials?.fallbackUrls?.[0]==='https://cherish.nfsa.go.kr/nfsa/releaseinformation/archive/materials/?pageIdx=1'&&nfsaMaterials?.detail===false,'official textbook monitoring uses the exact NFSA official materials paths and stays list-only to avoid unnecessary detail traffic');
