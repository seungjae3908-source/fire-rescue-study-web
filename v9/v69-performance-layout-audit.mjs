...
ok(sw.includes("v69-precomputed-v1")&&bundles.every(x=>sw.includes("'./"+x+"'")),'service worker precaches the seven V69 startup bundles under the precomputed cache generation');
...
ok(lazy.includes('questions-precomputed-v69-1.js')&&lazy.includes('for(const file of PRECOMPUTED_FILES)await loadScript(file)'),'six precomputed question chunks execute in deterministic order');
...
ok(lazy.includes('await loadScript(QUESTION_CORE_FILE)')&&lazy.includes('await loadScript(QUESTION_POST_FILE)')&&lazy.includes('Promise.all(QUESTION_FILES.map(loadScript))')&&lazy.includes('s.async=false'),'precomputed core finishes before the twenty deferred extension files execute');
