...
ok(executable.length===18,'eager executable scripts collapse from 101 to 18');
ok(executable[0]==='config.js'&&bundles.every(x=>executable.includes(x)),'config plus seventeen ordered runtime bundles are executable');
...
ok(lazy.includes('Promise.all(QUESTION_FILES.map(loadScript))')&&lazy.includes('s.async=false'),'twenty deferred question files fetch in parallel while classic-script execution order stays deterministic');
...
ok(runtimeLazy.includes('Promise.all(QUESTION_FILES.map(loadScript))')&&runtimeAi.includes('/1\\.5B.*Instruct/i')&&runtimeApp.includes('function wantsTutorNumber'),'active runtime bundles contain the same V69 question and AI safeguards');
