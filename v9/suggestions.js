...
async function list(){
  const {client}=await ready(),r=await client.from(T).select('*');if(r.error)throw r.error;
  return (r.data||[]).sort((a,b)=>Date.parse(b.created_at||0)-Date.parse(a.created_at||0))
}
