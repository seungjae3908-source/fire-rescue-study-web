...
try{
  ...
  const coverage=await p.evaluate(()=>window.AITUTOR_V9.contentPacks.coverage());assert(coverage.total===176&&coverage.verified===141&&coverage.pending===35,'browser runtime preserves 141 page-verified + 35 page-anchor-pending truth');
  const sprinklerAnchors=await p.evaluate(()=>{
    const V=window.AITUTOR_V9,spec={'F07-C05':284,'F07-C17':288,'F07-C18':284,'F07-C19':284,'F07-C20':302,'F07-C21':287};
  ...
  assert(sprinklerAnchors.every(x=>x.from===x.page&&x.status==='verified'&&x.precision==='exact-pdf-page-anchor'),'browser runtime exposes six verified sprinkler page anchors');
