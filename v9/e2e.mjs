...
try{
  ...
  const detailJump=await p.locator('.study-body-desktop .detail-toc button').first().getAttribute('data-book-jump');
  assert(!!detailJump&&await p.locator('#'+detailJump).count()===1,'detail outline points to a real textbook section');
