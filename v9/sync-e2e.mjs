...
try{
  ...
  const result=await page.evaluate(async()=>{
    ...
    await V.PrivateDocs.ingest(f,{kind:'personal',keepOriginal:false});
    ...
    return{
      ...
      exportSafe,imported,otherDocs:otherDocs.length,
  ...
  assert(result.syncPolicy.remoteFirstOnSignIn===true&&result.syncPolicy.originalFilesAutoUpload===false,'member sign-in is remote-first and original files never auto-upload');
  assert(result.privacyRules.crossUserSharing===false&&result.privacyRules.serverUpload===false,'local personal-document defaults remain no-share/no-auto-upload');
