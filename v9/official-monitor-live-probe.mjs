...
if(required.length<1||required.some(x=>!x.ok))throw new Error('OFFICIAL_MONITOR_REQUIRED_SOURCE_PARTIAL');
if(snapshot.coverageComplete!==true)console.warn('OFFICIAL_MONITOR_SUPPLEMENTAL_PARTIAL',JSON.stringify(snapshot.sourceStatus.filter(x=>!x.ok).map(x=>({id:x.id,errorCode:x.errorCode||'',error:x.error||''}))));
