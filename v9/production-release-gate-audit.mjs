...
const checks={
  ...
  acceptanceBrokerScopedCleanup:acceptanceBroker.includes('acceptance_broker===true')&&acceptanceBroker.includes('meta.acceptance_repo===REPOSITORY')&&acceptanceBroker.includes("client.from('study_suggestions').delete().eq('user_id',user.id)")&&acceptanceBroker.includes("client.from('study_admins').delete().eq('user_id',user.id)")&&acceptanceBroker.includes("client.from('study_memberships').delete().eq('user_id',user.id)")&&acceptanceBroker.includes('client.auth.admin.deleteUser(user.id)'),
...
const summary={version:'119-v26-production-release-gate-v17',checks,blockers,ready:blockers.length===0};
