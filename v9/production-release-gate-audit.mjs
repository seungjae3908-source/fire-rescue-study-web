...
const checks={
  ...
  acceptanceBrokerMembershipConstraintCompatible:acceptanceBroker.includes("client.from('study_memberships').insert([{user_id:id,source:'study-v9'}])")&&!acceptanceBroker.includes("source:'production-acceptance-oidc'"),
  acceptanceBrokerEphemeralUsers:acceptanceBroker.includes('client.auth.admin.createUser')&&acceptanceBroker.includes('email_confirm:true')&&acceptanceBroker.includes("app_scope:'study-v9-qa'")&&acceptanceBroker.includes('acceptance_run_id:runId')&&acceptanceBroker.includes("client.from('study_memberships').insert")&&acceptanceBroker.includes("client.from('study_admins').insert"),
  acceptanceBrokerScopedCleanup:acceptanceBroker.includes('acceptance_broker===true')&&acceptanceBroker.includes('meta.acceptance_repo===REPOSITORY')&&acceptanceBroker.includes("String(meta.acceptance_run_id||'')===runId")&&acceptanceBroker.includes('client.auth.admin.deleteUser(user.id)')&&!acceptanceBroker.includes("client.from('study_suggestions').delete().eq('user_id',user.id)"),
...
const summary={version:'119-v26-production-release-gate-v18',checks,blockers,ready:blockers.length===0};
