...
for(const[id,qid]of Object.entries(promotedExpected)){
  ...
  if(verified.length<3)throw new Error('V44_BATCH7_VERIFIED_COUNT_REGRESSION '+id+' '+verified.length+' expected_at_least 3');
...
for(const[id,count]of Object.entries(blockerExpected)){
  ...
  if(verified.length<count)throw new Error('V44_BATCH7_BLOCKER_COUNT_REGRESSION '+id+' '+verified.length+' expected_at_least '+count);
...
// V44 is a historical regression gate; later V60 promotions are checked separately.
