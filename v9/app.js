...
const id=`F05-C${String(Number(n)+1).padStart(2,'0')}`,open=String(runtime.hazmatClassOpen)===String(n),rows=V.Hazmat2026.itemRows(Number(n)),p=V.contentPacks?.get?.(id)||{},must=(p.must||[]).slice(0,3),trap=(p.traps||[])[0]||'';
return `<article class="hazmat-class-card ${open?'open':''}"><button class="hazmat-class-summary" data-hazmat-class-toggle="${n}" aria-expanded="${open?'true':'false'}"><span><b>제${n}류 · ${esc(x.nature)}</b><small>${x.items.length}개 품명군 · 눌러서 전체 보기</small></span><i aria-hidden="true">${open?'−':'+'}</i></button>${open?`<div class="hazmat-class-detail"><div class="hazmat-item-list">${rows.map(r=>`<div><span>${esc(r.name)}</span><b>${Number(r.designated).toLocaleString('ko-KR')} ${esc(r.unit)}</b></div>`).join('')}</div>${must.length?`<div class="hazmat-brief"><b>공통 성질 · 소화 포인트</b><ul>${must.map(v=>`<li>${esc(studentStudyText(v))}</li>`).join('')}</ul></div>`:''}${trap?`<p class="hazmat-trap"><b>시험 주의</b> ${esc(studentStudyText(trap))}</p>`:''}<button class="btn small primary" data-concept="${id}">제${n}류 자세히 학습</button></div>`:''}</article>`
}
function hazmatBlock(c){
if(c.scopeId!=='F05'||!V.Hazmat2026)return'';
const idx=Number(c.id.split('-C')[1]),classNo=idx>=2&&idx<=7?idx-1:null;
if(idx===1){return `<section class="hazmat-reference"><div class="lesson-heading"><div><span class="eyebrow">위험물 분류</span><h3>제1류~제6류 품명 · 지정수량</h3><p class="muted">각 류를 누르면 실제 품명군과 지정수량, 공통 성질과 주의점을 바로 확인할 수 있습니다.</p></div></div><div class="hazmat-class-grid">${Object.entries(V.Hazmat2026.classes).map(([n,x])=>hazmatOverviewCard(n,x)).join('')}</div></section>`}
...
if(cardTypes.has(type)&&pack.compare.length<=12)return `<section class="detail-compare detail-compare-cards"><h3>${esc(title)}</h3><div class="concept-class-grid">${pack.compare.map(r=>`<details class="concept-class-card"><summary>${esc(studentStudyText(r[0]))}</summary><p>${esc(studentStudyText(r[1]))}</p></details>`).join('')}</div></section>`;
return `<section class="detail-compare"><h3>${esc(title)}</h3><div class="compare-wrap"><table class="compare"><thead><tr><th>구분</th><th>내용 · 핵심 차이</th></tr></thead><tbody>${pack.compare.map(r=>`<tr><td><b>${esc(studentStudyText(r[0]))}</b></td><td>${esc(studentStudyText(r[1]))}</td></tr>`).join('')}</tbody></table></div></section>`
}
...
if(tab==='detail'){const coreSeeds=coreStudySeeds(pack),rawRows=[...schemaDetailRows(c,pack),...detailGroups(c,detail,coreSeeds),...sections],detailRows=rawRows.filter(x=>detailHasUniqueContent(x,coreSeeds));return `<div class="lesson detail-view detail-type-${esc(V.ConceptArchitecture119?.typeOf?.(c.id)||'general')}">${detailDefinitionBlock(c,pack)}${comparisonBlock(c,pack)}${detailRows.map((x,i)=>detailSection(c,x,i,coreSeeds)).join('')}${detailExamPointBlock(pack)}${visualBlocks(pack)}${hazmatBlock(c)}${specialCombustibleBlock(pack)}${calculationBlocks(c,pack)}</div>`}
if(tab==='quiz'){
...
function exam(){const ready=V.examReadiness(),past=V.OfficialPastExam119, pastN=past?.questions?.length||0;if(runtime.exam)return examRun();const wrongN=state().wrongs.filter(w=>!w.resolved).length;return shell(`<div class="exam-landing screen-scroll"><section class="exam-start card"><span class="eyebrow">실전형 문제 구성</span><h2>실전 모의고사</h2><div class="exam-spec"><b>65문항 · 65분</b><span>소방학개론 25문항</span><span>응급처치학개론 40문항</span></div><p class="exam-quality-note">공식 교재 근거가 확인된 문항과 현재 기준에 호환되는 공식 기출을 우선해 구성합니다. 같은 개념·유형의 과도한 반복과 정답 위치 편향을 줄입니다.</p><div class="difficulty-picker"><b>난이도</b><div class="seg">${[['low','하'],['mid','중'],['high','상']].map(([k,l])=>`<button class="${runtime.examDifficulty===k?'on':''}" data-exam-difficulty="${k}">${l}</button>`).join('')}</div></div>${ready.ready?'<button class="btn primary block" data-exam-start="real">실전 모의고사 시작</button>':'<p class="muted exam-wait">실전 문제은행을 준비 중입니다. 지금은 연습 모의고사를 이용할 수 있습니다.</p>'}<button class="btn block" data-exam-start="practice" style="margin-top:8px">연습 모의고사 시작</button>${past?.ready?`<div class="official-past-exam-card"><div><span class="eyebrow">소방청 공개 실제 기출</span><b>2025 기출 · ${pastN}문항</b><small>그림·보기 자료가 없어도 문항 자체로 완결되는 문제만 수록했습니다. 시행 당시 정답 기준입니다.</small></div><button class="btn primary block" data-exam-start="past2025">2025 실제 기출 풀기</button><a href="${esc(past.sourceUrl)}" target="_blank" rel="noopener">소방청 원문·가답안 확인 ↗</a></div>`:''}</section><section class="card training-center"><div><span class="eyebrow">대용량 반복훈련</span><h2>집중 문제훈련</h2><p class="muted">실전 65문제 형식과 분리된 연습 전용입니다. 풀이 후 오답·취약단원 분석은 동일하게 제공합니다.</p></div><div class="training-group"><b>과목 집중</b><div class="training-grid"><button class="btn" data-training-start="fire50">소방 50</button><button class="btn" data-training-start="fire100">소방 100</button><button class="btn" data-training-start="ems80">구급 80</button><button class="btn" data-training-start="ems120">구급 120</button></div></div><div class="training-group"><b>전범위 대용량</b><div class="training-grid three"><button class="btn" data-training-start="all100">100문제</button><button class="btn" data-training-start="all150">150문제</button><button class="btn" data-training-start="all200">200문제</button></div></div><div class="training-group"><b>약점 집중</b><small class="muted">현재 미해결 오답 ${wrongN}개</small><div class="training-grid three"><button class="btn" data-training-start="wrong20">오답 기반 20</button><button class="btn" data-training-start="wrong50">오답 기반 50</button><button class="btn" data-training-start="wrong100">오답 기반 100</button></div><button class="btn primary block" data-training-start="weak">취약개념 자동시험</button></div><button class="btn ghost block" data-calc-bank>계산 문제 훈련</button></section></div>`,'시험')}
...
function examRun(){
const e=runtime.exam,q=e.qs[e.i],real=e.mode==='real',training=e.mode==='training',past=e.mode==='past',total=e.qs.length,left=real?Math.max(0,3900-Math.floor((Date.now()-e.startedAt)/1000)):null,answered=Object.keys(e.answers).length,remaining=Math.max(0,total-answered),label=real?'실전 모의고사':past?(e.title||'실제 기출'):training?(e.title||'집중훈련'):'연습 모의고사',pct=Math.round(answered/Math.max(1,total)*100);
...
const rec={
id:e.id,mode:e.mode,difficulty:e.difficulty,at:Date.now(),score:scoreDen?Math.round(correct/scoreDen*100):0,correct,total,totalAnswered:answered,unanswered,elapsedSec,
fireCorrect,emsCorrect,fireAnswered,emsAnswered,questionIds:e.qs.map(q=>q.id),answers:answerSnapshot,confidence:confidenceSnapshot,incorrectQuestionIds,unansweredQuestionIds,
blueprint:e.blueprint||null,partial,abandoned,completionRate:total?Math.round(answered/total*100):0,scoreBasis:partial?'answered':'all',detailVersion:2
};
...
const partial=rec.partial===true||rec.abandoned===true,ids=rec.questionIds||[],answers=rec.answers||{},items=ids.map(id=>{const q=V.questionById[id];return q?{q,selected:answers[id]}:null}).filter(Boolean),answeredItems=items.filter(x=>x.selected!==undefined),analysisItems=partial?answeredItems:items;
...
if(b.dataset.passCore){try{const r=await V.PassNote.toggleConceptCore(b.dataset.passCore);toast(r.saved?'단원 핵심을 합격노트에 저장':'합격노트에서 단원 핵심 해제');return render()}catch(err){return toast('합격노트 저장 실패 · '+String(err?.message||err).slice(0,40))}}
if(b.dataset.passStar){try{const r=await V.PassNote.toggleConcept(b.dataset.passStar);toast(r.saved?'합격노트에 저장됨':'합격노트에서 해제됨');return render()}catch(err){return toast('합격노트 저장 실패 · '+String(err?.message||err).slice(0,40))}}
...
if(b.dataset.goalRemove){V.Mastery.dailyGoalRemove(b.dataset.goalRemove);toast('오늘 목표에서 제거됨');return render()}
