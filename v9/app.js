...
const ids=V.Mastery.todayPlan(Math.min(40,V.curriculum.concepts.length)).map(x=>x.concept.id),pool=trainingPool('all').filter(q=>ids.includes(q.conceptId));
...
return shell(`<div class="exam-landing screen-scroll"><section class="exam-start card"><span class="eyebrow">실전형 문제 구성</span><h2>실전 모의고사</h2><div class="exam-spec"><b>65문항 · 65분</b><span>소방학개론 25문항</span><span>응급처치학개론 40문항</span></div><p class="exam-quality-note">실전은 A/B급 검증문항만 사용하고, 같은 회차·난이도는 같은 문제지로 다시 열립니다. 최근 노출은 문제 변형 family 단위로 억제합니다.</p>${roundPicker}<div class="difficulty-picker"><b>난이도</b><div class="seg">${[['low','하'],['mid','중'],['high','상']].map(([k,l])=>`<button class="${runtime.examDifficulty===k?'on':''}" data-exam-difficulty="${k}">${l}</button>`).join('')}</div></div>${ready.ready?`<button class="btn primary block" data-exam-start="real">실전 ${round}회 시작</button>`:'<p class="muted exam-wait">실전 문제은행을 준비 중입니다. 지금은 연습 모의고사를 이용할 수 있습니다.</p>'}<button class="btn block" data-exam-start="practice" style="margin-top:8px">연습 ${round}회 시작 · 안전 변형</button>${practiceSaved?`<button class="btn small ghost block exam-rebuild-btn" data-exam-round-reset="practice">연습 ${round}회 새 문제로 재구성</button>`:''}${past?.ready?`<div class="official-past-exam-card"><div><span class="eyebrow">소방청 공개 실제 기출</span><b>2025 기출 · ${pastN}문항</b><small>그림·보기 자료가 없어도 문항 자체로 완결되는 문제만 수록했습니다. 시행 당시 정답 기준입니다.</small></div><button class="btn primary block" data-exam-start="past2025">2025 실제 기출 풀기</button><a href="${esc(past.sourceUrl)}" target="_blank" rel="noopener">소방청 원문·가답안 확인 ↗</a></div>`:''}</section><section class="card training-center"><div><span class="eyebrow">대용량 반복훈련</span><h2>집중 문제훈련</h2><p class="muted">연습·훈련은 검증된 원문 사실을 바꾸지 않고 보기 순서 또는 계산 변수만 안전하게 변형합니다.</p></div><div class="training-group"><b>과목 집중</b><div class="training-grid"><button class="btn" data-training-start="fire50">소방 50</button><button class="btn" data-training-start="fire100">소방 100</button><button class="btn" data-training-start="ems80">구급 80</button><button class="btn" data-training-start="ems120">구급 120</button></div></div><div class="training-group"><b>전범위 대용량</b><div class="training-grid three"><button class="btn" data-training-start="all100">100문제</button><button class="btn" data-training-start="all150">150문제</button><button class="btn" data-training-start="all200">200문제</button></div></div><div class="training-group"><b>약점 집중</b><small class="muted">현재 미해결 오답 ${wrongN}개</small><div class="training-grid three"><button class="btn" data-training-start="wrong20">오답 기반 20</button><button class="btn" data-training-start="wrong50">오답 기반 50</button><button class="btn" data-training-start="wrong100">오답 기반 100</button></div><button class="btn primary block" data-training-start="weak65">취약점 맞춤 65</button><button class="btn ghost block" data-training-start="weak">취약개념 빠른 50</button></div><button class="btn ghost block" data-calc-bank>계산 문제 훈련</button></section></div>`,'시험')
}
