...
const SOURCE_OVERLAY_HISTORY='aitutorV9SourceOverlay';
function sourceOverlayRoot(){return document.querySelector('#pdfEvidence,#resourcePdf,#sourceModal')}
function pushSourceOverlayHistory(kind){
  const current=history.state&&typeof history.state==='object'?history.state:{};
  if(current[SOURCE_OVERLAY_HISTORY])return;
  history.pushState({...current,[SOURCE_OVERLAY_HISTORY]:kind||true},'')
}
function closeSourceOverlay(root=sourceOverlayRoot(),fromHistory=false){
  if(!root)return false;
  root.remove();
  if(!fromHistory&&history.state?.[SOURCE_OVERLAY_HISTORY])history.back();
  return true
}
function closeSourceOverlayById(id,fromHistory=false){return closeSourceOverlay(document.querySelector(id),fromHistory)}
window.addEventListener('popstate',()=>{
  if(closeSourceOverlay(sourceOverlayRoot(),true))return;
  if(history.state?.[SOURCE_OVERLAY_HISTORY]){
    const next={...history.state};delete next[SOURCE_OVERLAY_HISTORY];history.replaceState(next,'')
  }
});
document.addEventListener('keydown',e=>{if(e.key==='Escape'&&sourceOverlayRoot()){e.preventDefault();e.stopPropagation();closeSourceOverlay()}})
...
async function openPdfEvidence(id){const c=V.curriculum.byId[id],pack=V.contentPacks?.get?.(id),range=(c?.sourceRanges||[]).find(x=>x?.doc)||(c?.sourceRanges||[])[0],key=range?.doc||'',bookFrom=Number(range?.from)||0;
  if(!key){
    ...
    document.body.insertAdjacentHTML('beforeend',`<div class="modal-wrap" id="sourceModal" data-source-backdrop><div class="modal source-modal"><div class="toolbar pdf-modal-head source-modal-head"><div><span class="eyebrow">공식 웹 근거</span><h2>${esc(c?.title||id)}</h2></div><span class="spacer"></span><div class="pdf-nav-actions"><button class="btn small ghost pdf-back-btn" data-source-close data-source-nav="back" aria-label="원문 보기에서 뒤로가기">← 뒤로</button><button class="btn small ghost pdf-close-btn" data-source-close data-source-nav="close" aria-label="원문 보기 닫기">닫기 ✕</button></div></div><p class="lead">${esc(pack?.source||'공식 근거')}</p><div class="source-law-links">${links.map(x=>`<a class="source-law-link" href="${esc(x.url)}" target="_blank" rel="noopener">${esc(x.label||'공식 원문')} <span aria-hidden="true">↗</span></a>`).join('')}</div></div></div>`);
    pushSourceOverlayHistory('source-web');
    return
  }
  const hasMappedBookPage=Object.prototype.hasOwnProperty.call(V.SourcePDF?.pageOffsets||{},key),initialPdf=bookFrom?(V.SourcePDF.pdfPage?.(key,bookFrom)||bookFrom):0;document.querySelector('#sourceModal')?.remove();document.querySelector('#pdfEvidence')?.remove();document.body.insertAdjacentHTML('beforeend',`<div class="modal-wrap" id="pdfEvidence" data-pdf-backdrop data-concept-id="${esc(id)}" data-doc-key="${esc(key)}" data-page="${initialPdf||''}" data-zoom="1"><div class="modal pdf-evidence-modal"><div class="toolbar pdf-modal-head"><div><span class="eyebrow">공식 근거</span><h2>${esc(c?.title||id)}</h2><small data-pdf-page-label class="muted">${bookFrom?(hasMappedBookPage?`교재 ${bookFrom}쪽`:`PDF ${bookFrom}쪽`):'근거 위치 찾기'}</small></div><span class="spacer"></span><div class="pdf-nav-actions"><button class="btn small ghost pdf-back-btn" data-pdf-close data-source-nav="back" aria-label="원문 보기에서 뒤로가기">← 뒤로</button><button class="btn small" data-pdf-download>다운로드</button><button class="btn small ghost pdf-close-btn" data-pdf-close data-source-nav="close" aria-label="원문 보기 닫기">닫기 ✕</button></div></div><div class="toolbar pdf-zoombar"><button class="btn small ghost" data-pdf-zoom="-0.25">−</button><span class="pill" data-pdf-zoom-label>100%</span><button class="btn small ghost" data-pdf-zoom="0.25">＋</button><button class="btn small ghost" data-pdf-fit>폭 맞춤</button></div><div id="pdfEvidenceHost" class="pdf-evidence-host"><div class="empty">공식 원문 여는 중…</div></div><div class="toolbar pdf-pager hidden"><button class="btn ghost" data-pdf-page="-1">← 이전 페이지</button><button class="btn ghost" data-pdf-page="1">다음 페이지 →</button></div></div></div>`);pushSourceOverlayHistory('evidence-pdf');await renderPdfEvidence(id)}
document.addEventListener('click',async e=>{const t=e.target instanceof Element?e.target:null;if(!t)return;if(t.matches('[data-backdrop-close-more]')){runtime.more=false;return render()}if(t.matches('[data-backdrop-close-account]')){runtime.account=false;return render()}if(t.matches('[data-source-backdrop]')){closeSourceOverlay(t);return}if(t.matches('[data-resource-pdf-backdrop]')){closeSourceOverlay(t);return}if(t.matches('[data-pdf-backdrop]')){closeSourceOverlay(t);return}const b=t.closest('button,[data-outline-close]');if(!b)return;if('resourcePdfClose'in b.dataset){closeSourceOverlayById('#resourcePdf');return}
