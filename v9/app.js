...
const t=String(v||''),a=coreHighlightTerms(pack).filter(x=>t.includes(x)).slice(0,2),n=(t.match(/\d+(?:\.\d+)?(?:\s*(?:~|–|-)\s*\d+(?:\.\d+)?)?\s*(?:초|분|시간|cm|mm|m|kg|L|%|J\/kg)?/g)||[]).slice(0,2),terms=[...new Set([...a,...n].filter(Boolean))].slice(0,3).sort((x,y)=>y.length-x.length);
...
return `<article class="hazmat-class-card ${open?'open':''}"><button class="hazmat-class-summary" data-hazmat-class-toggle="${n}" aria-expanded="${open?'true':'false'}"><span><b>제${n}류 · ${esc(x.nature)}</b><small>${x.items.length}개 품명군 · 눌러서 전체 보기</small></span><i aria-hidden="true">${open?'−':'+'}</i></button>${open?`<div class="hazmat-class-detail"><div class="hazmat-item-list">${rows.map(r=>`<div><span>${esc(r.name)}</span><b>${Number(r.designated).toLocaleString('ko-KR')} ${esc(r.unit)}</b></div>`).join('')}</div>${must.length?`<div class="hazmat-brief"><b>공통 성질 · 소화 포인트</b><ul>${must.map(v=>`<li>${esc(v)}</li>`).join('')}</ul></div>`:''}${trap?`<p class="hazmat-trap"><b>시험 주의</b> ${esc(trap)}</p>`:''}<button class="btn small primary" data-concept="${id}">제${n}류 자세히 학습</button></div>`:''}</article>`
}
...
function quickCoreBlock(pack){const text=pack?.studySchema?.quick30||pack?.summary||'';if(!text)return'';return `<section class="study-quick"><div class="study-quick-title">핵심</div><p class="lead">${studyHighlight(text,pack)}</p></section>`}
function coreEssentialBlock(c,pack){const rows=coreEssentialRows(pack);if(!rows.length)return'';return `<section class="study-core-essentials"><div class="study-core-title"><span>시험 직전 핵심</span><button class="study-note-link" data-pass-note-open>합격노트 보기</button></div><ul>${rows.map(x=>{const key=V.PassNote?.conceptKey?.(c.id,x.bucket,x.index)||'',on=key&&V.PassNote?.has?.(key);return `<li><button class="study-star-btn ${on?'on':''}" data-pass-star="${esc(key)}" aria-label="합격노트 ${on?'해제':'저장'}">${on?'★':'☆'}</button><span>${esc(x.text)}</span></li>`}).join('')}</ul></section>`}
function numberBlock(c,pack){const rows=V.StudyEmphasis119?.numberRows?.(pack,12)||[];if(!rows.length)return'';return `<section class="study-numbers"><div class="study-numbers-title"><span>숫자 · 단위 · 기준</span><button class="study-note-link" data-pass-note-open>합격노트 보기</button></div><ul>${rows.map((x,i)=>{const key=V.PassNote?.conceptKey?.(c.id,'number',i)||'',on=key&&V.PassNote?.has?.(key);return `<li><button class="study-star-btn ${on?'on':''}" data-pass-star="${esc(key)}" aria-label="합격노트 ${on?'해제':'저장'}">${on?'★':'☆'}</button><span class="study-key-text">${studyHighlight(x,pack)}</span></li>`}).join('')}</ul></section>`}
