...
function lessonContent(c,pack,tab){const qs=V.QuestionQuality119?.forConcept(c.id)||[],detail=pack.detail||[],sections=uniqueSections(pack.deepSections||[]);
  if(tab==='detail'){const detailRows=sections.length?sections:uniqueTextRows(detail,pack.summary).map((body,i)=>({title:i===0?'정의 · 상세':'상세 정리',body,bullets:[]}));return `<div class="lesson detail-view">${studySchemaBlock(pack)}${detailRows.map(detailSection).join('')}${numberBlock(c,pack)}${visualBlocks(pack)}${hazmatBlock(c)}${specialCombustibleBlock(pack)}${calculationBlocks(c,pack)}${comparisonBlock(pack)}${trapBlock(pack)}</div>`}
  if(tab==='quiz'){
...
function tutorConceptFor(prompt){
  ...
  for(const concept of V.curriculum?.concepts||[]){
    ...
    for(const term of new Set(terms)){
      if(q.includes(term))score+=term.length*10+(term===studyNorm(raw)?80:0);
    }
...
function aiChatBody(c){
  const chat=state().chat.slice(-12);
  return `<div class="study-ai"><div class="study-ai-head"><div><span class="eyebrow">AI 질문</span><b>질문 내용에 맞는 개념을 먼저 찾아 답합니다.</b></div><span class="tiny muted">${esc(runtime.aiStatus)}</span></div><div class="study-ai-chat" id="chatBox">${chat.length?chat.map(m=>`<div class="row tutor-message ${m.role==='user'?'me':'assistant'}"><b>${m.role==='user'?'나':'119'}</b><p>${esc(m.text)}</p></div>`).join(''):`<div class="tutor-empty compact"><p><b>${esc(c.title)}</b>을 공부 중입니다. 다른 개념을 질문해도 질문 자체를 우선해서 답합니다.</p><div class="tutor-quick"><button data-tutor-prompt="이 개념을 시험 직전 30초 요약으로 설명해줘">30초 요약</button><button data-tutor-prompt="이 개념과 헷갈리는 개념을 비교해줘">헷갈리는 비교</button></div></div>`}</div><div class="tutor-compose"><input data-tutor-input class="input" placeholder="예: 플래시오버에 대해 알려줘"><button class="btn primary" data-tutor-send>보내기</button></div>${!(runtime.aiEngine||V.LocalAI?.ready)?'<button class="btn small ghost ai-load-inline" data-ai-load>로컬 AI 사용</button>':''}</div>`
}
...
async function sendTutor(){const input=visibleTutorInput(),prompt=input?.value.trim();if(!prompt)return;const current=currentConcept(),c=tutorConceptFor(prompt),pack=V.contentPacks.get(c.id),privateText=await V.PrivateDocs.privateGrounding(prompt,4).catch(()=>''),userMsg={id:crypto.randomUUID?crypto.randomUUID():'chat-u-'+Date.now(),role:'user',text:prompt,at:Date.now()},assistant={id:crypto.randomUUID?crypto.randomUUID():'chat-a-'+Date.now(),role:'assistant',text:'생각 중…',at:Date.now()};state().chat.push(userMsg,assistant);S.save();render();let out=fallbackTutor(prompt,c,pack,privateText);if(runtime.aiEngine||V.LocalAI?.ready){try{out=await V.LocalAI.chat([{role:'system',content:'너는 119 학습도우미다. 반드시 사용자의 질문 대상 개념을 우선해 답한다. 현재 화면의 학습 개념은 질문이 모호할 때만 보조 문맥으로 사용한다. 제공된 공식 학습팩과 개인자료 밖의 사실을 단정하지 않고, 숫자·법규·의학 기준은 근거가 없으면 모른다고 말한다.'},{role:'user',content:`[현재 화면]\n${current.id} ${current.title}\n[질문 대상]\n${c.id} ${c.title}\n[질문 대상 공식 학습팩]\n${(pack.detail||[]).join('\n')}\n[시험필수]\n${(pack.must||[]).join('\n')}\n[개인자료]\n${privateText||'없음'}\n[질문]\n${prompt}\n\n질문 대상을 먼저 직접 설명하고, 필요할 때만 현재 화면 개념과 연결해. 쉬운 설명 → 시험포인트 → 헷갈리는 비교 → 확인질문 순서로 답하고 근거 부족은 명시해.`}],{temperature:.1,max_tokens:850})||out;runtime.aiEngine=V.LocalAI.engine||runtime.aiEngine}catch{}}state().chat[state().chat.length-1]={...assistant,text:out,targetConceptId:c.id};S.save();render()}
function tutor(){const c=currentConcept();return shell(`<div class="tutor-standalone">${aiChatBody(c)}</div>`,'AI 질문',`${c.scopeTitle} › ${c.title}`)}
...
function refreshSuggestionsView(){
  ...
  if(count)count.textContent=runtime.suggestionsLoading?'불러오는 중':runtime.suggestions.length+'건'
}
async function ensureSuggestions(force=false){
  const owner=V.Auth?.user?.id||'guest';
  if(owner==='guest'){runtime.suggestions=[];runtime.suggestionsAdmin=false;runtime.suggestionsOwner='guest';runtime.suggestionsLoading=false;refreshSuggestionsView();return}
  ...
  }catch(err){runtime.suggestions=[];runtime.suggestionsAdmin=false;runtime.suggestionsOwner=owner;runtime.suggestionError='건의함을 불러오지 못했습니다: '+String(err?.message||err).slice(0,70)}
...
function suggestions(){
  ...
  <section class="card"><div class="toolbar"><b>${admin?'전체 건의사항':'내 건의사항'}</b><span class="spacer"></span><span class="tiny muted" data-suggestion-count>${runtime.suggestionsLoading?'불러오는 중':runtime.suggestions.length+'건'}</span></div><div class="suggestion-list">${runtime.suggestionsLoading?'<div class="empty" style="height:100px">불러오는 중…</div>':suggestionRowsHtml()}</div></section></div>`,'건의함')
}
...
if('suggestSubmit'in b.dataset){const category=$('#suggestCategory')?.value||'개선',title=$('#suggestTitle')?.value.trim()||'',body=$('#suggestBody')?.value.trim()||'',anonymous=$('#suggestAnonymous')?.checked!==false;runtime.suggestionDraft={category,title,body,anonymous};runtime.suggestionError='';if(title.length<2){runtime.suggestionError='제목을 2자 이상 입력해주세요.';return render()}if(body.length<2){runtime.suggestionError='건의 내용을 2자 이상 입력해주세요.';return render()}try{await V.Suggestions.create({category,title,body,anonymous});runtime.suggestionDraft={category:'개선',title:'',body:'',anonymous:true};runtime.suggestionError='';runtime.suggestionsOwner='';await ensureSuggestions(true);toast('건의사항이 접수되었습니다.');return}catch(err){const code=String(err?.message||err);runtime.suggestionError=code==='TITLE_REQUIRED'?'제목을 2자 이상 입력해주세요.':code==='BODY_REQUIRED'?'건의 내용을 2자 이상 입력해주세요.':'접수하지 못했습니다. 잠시 후 다시 시도해주세요. ('+code.slice(0,40)+')';return render()}}
