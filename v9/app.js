...
function docsListHtml(){return runtime.docs.length?runtime.docs.map(d=>{const review=(d.reviewPages||[]).length;return `<div class="row doc-row"><div><b>${esc(d.title)}</b><small>${d.pageCount}쪽${review?` · 원문 확인 필요 ${review}쪽`:''}</small></div><div class="toolbar"><button class="btn small" data-doc-open="${d.id}">내용 보기</button><button class="btn small primary" data-doc-pass="${d.id}">AI 합격노트 초안</button><button class="btn small ghost" data-doc-delete="${d.id}">삭제</button></div></div>`}).join(''):'<div class="empty" style="height:100px">등록된 자료가 없습니다.</div>'}
...
<section class="card"><div class="toolbar"><b>PDF / 사진에서 노트 만들기</b><span class="spacer"></span><label class="btn small primary">+ PDF / 사진 추가<input id="personalFile" type="file" accept="application/pdf,image/*,.txt" hidden></label></div><p class="tiny muted">PDF·사진에서 학습할 내용을 추려 합격노트 초안으로 정리합니다. 중요한 숫자·단위는 원문과 함께 확인해 주세요.</p><small class="upload-state" data-upload-status>${esc(runtime.uploadStatus||'')}</small></section>
...
document.addEventListener('change',async e=>{if(e.target.dataset.detailJumpSelect!==undefined){const value=String(e.target.value||'');if(value!==''){const root=e.target.closest('.study-body')||document,target=root.querySelector(`[data-detail-section="${CSS.escape(value)}"]`);target?.scrollIntoView({behavior:'smooth',block:'start'});e.target.value=''}return}if(e.target.id==='personalFile'){const f=e.target.files?.[0];if(!f)return;runtime.uploadStatus='자료 확인 준비 중';const update=(t)=>{runtime.uploadStatus=t;const el=document.querySelector('[data-upload-status]');if(el)el.textContent=t};update('자료 확인 중…');try{const result=await V.PrivateDocs.ingest(f,{kind:'personal',keepOriginal:true,aiAssist:'auto',onProgress:(i,n)=>update(`자료 확인 중 · ${i}/${n}쪽`)});runtime.docsOwner='';await ensureDocs();const review=result.reviewPages?.length||0;update(review?`분석 완료 · 원문 확인 필요 ${review}쪽`:'분석 완료');toast(review?`자료 분석 완료 · ${review}쪽 확인 필요`:'자료 분석 완료')}catch(err){update('분석 실패 · 오류 내용을 확인해주세요');toast('자료 처리 실패: '+String(err.message||err).slice(0,70))}}if(e.target.id==='importBackup'){const f=e.target.files?.[0];if(!f)return;try{S.import(await f.text());toast('복원 완료');render()}catch{toast('복원 실패')}}});
