...
<div class="metric-grid dashboard-metrics"><div class="metric"><span>전체 학습 진도</span><b>${r.scopeCoverage}%</b></div><div class="metric"><span>복습 예정</span><b>${r.overdue}</b></div><div class="metric"><span>최근 오답</span><b>${wrongN}</b></div><div class="metric"><span>최근 시험</span><b>${last?last.score+'점':'-'}</b></div></div>
...
</div><aside class="home-side"><section class="card dashboard-quick"><b>바로가기</b><div class="dashboard-quick-grid"><button class="btn ghost" data-go="resources">공식 자료</button><button class="btn ghost" data-go="wrong">오답 복습</button><button class="btn ghost" data-go="stats">성적 분석</button></div></section><section class="card motivation-card"><b>오늘 한 줄</b><p>${esc(motivation)}</p></section></aside></div>`,'홈')
}
function outline(){
...
function study(){const c=currentConcept(),sc=currentScope(),pack=V.contentPacks.get(c.id),tab=STUDY_TABS.some(([k])=>k===state().studyTab)?state().studyTab:'core',nb=conceptNeighbors(c);if(state().studyTab!==tab){state().studyTab=tab;S.save()}return shell(`<div class="study" data-scroll-owner="study"><div class="study-toolbar"><button class="btn small" data-outline>목차</button><div class="seg"><button class="${state().subject==='fire'?'on':''}" data-subject="fire">소방학</button><button class="${state().subject==='ems'?'on':''}" data-subject="ems">응급처치</button></div><span class="path">${esc(sc.title)} › ${esc(c.title)}</span></div><div class="workspace study-workspace-single">${outline()}<div class="study-mainpane"><header class="concept-head"><span class="scope-label">${esc(sc.title)}</span><h2>${esc(c.title)}</h2><div class="tabbar">${STUDY_TABS.map(([k,l])=>`<button class="${tab===k?'on':''}" data-study-tab="${k}">${l}</button>`).join('')}</div></header><div class="study-body study-body-desktop">${lessonContent(c,pack,tab)}</div><div class="study-body study-body-mobile">${lessonBook(c,pack)}</div><footer class="actionbar concept-nav"><button class="btn" data-study-prev ${nb.prev?'':'disabled'}>← 이전</button><button class="btn ghost" data-outline>목차</button><button class="btn primary" data-study-next ${nb.next?'':'disabled'}>다음 →</button></footer></div></div></div>`,'학습')}
