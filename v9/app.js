...
function studentStudyText(v){return String(v||'').replace(/개념\s*구조와\s*읽는\s*순서/gi,'').replace(/\b20\d{2}\s*(?:소방전술\s*\d+(?:\([^)]*\))?|예방실무\s*\d+)\s*기준으로\s*/gi,'').replace(/\s*교재의\s*정의(?:이)?다\.?/gi,'').replace(/\[\s*\]/g,'').replace(/\s{2,}/g,' ').trim()}
function studentQuestionText(v){return studentStudyText(v).replace(/([“\"]?)\s*(?:교재|공식\s*교재)에서는?\s*/gi,'$1').replace(/\s{2,}/g,' ').trim()}
...
function numberBlock(c,pack){const rows=V.StudyEmphasis119?.numberRows?.(pack,12)||[];if(!rows.length)return'';return `<section class="study-numbers"><div class="study-numbers-title"><span>숫자 · 단위 · 기준</span></div><ul>${rows.map(x=>`<li><span class="study-key-text">${esc(x)}</span></li>`).join('')}</ul></section>`}
