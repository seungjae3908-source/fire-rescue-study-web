...
</div><aside class="home-side"><section class="card dashboard-quick"><b>빠른 이동</b><div class="dashboard-quick-actions"><button class="btn block" data-go="wrong">오답 복습</button><button class="btn block ghost" data-go="resources">공식 자료</button></div></section><section class="card motivation-card"><b>오늘 한 줄</b><p>${esc(motivation)}</p></section></aside></div>`,'홈')
}
function outline(){
...
let raf=0;
const update=()=>{raf=0;const locked=owner.dataset.detailJumpLock;if(locked){syncDetailTocActive(view,locked);return}const r0=owner.getBoundingClientRect(),anchor=r0.top+Math.min(120,Math.max(72,owner.clientHeight*.20));let best=sections[0],bestScore=Infinity;for(const sec of sections){const r=sec.getBoundingClientRect(),passed=r.top<=anchor,score=(passed?0:500)+Math.abs(r.top-anchor);if(score<bestScore){best=sec;bestScore=score}}syncDetailTocActive(view,best.dataset.detailSection)};
