...
function lessonContent(c,pack,tab){const qs=V.QuestionQuality119?.forConcept(c.id)||[],detail=pack.detail||[],sections=uniqueSections(pack.deepSections||[]);
  if(tab==='detail'){const coreSeeds=coreStudySeeds(pack),rawRows=[...schemaDetailRows(pack),...detailGroups(detail,coreSeeds),...sections],detailRows=rawRows.filter(x=>detailHasUniqueContent(x,coreSeeds));return `<div class="lesson detail-view">${detailDefinitionBlock(c,pack)}${comparisonBlock(c,pack)}${detailToc(detailRows)}${detailRows.map((x,i)=>detailSection(x,i,coreSeeds)).join('')}${visualBlocks(pack)}${hazmatBlock(c)}${specialCombustibleBlock(pack)}${calculationBlocks(c,pack)}</div>`}
  if(tab==='quiz'){
