...
function home(){
  const r=V.Mastery.readiness(),plan=V.Mastery.todayPlan(6),last=state().examHistory.slice(-1)[0],wrongN=state().wrongs.filter(x=>!x.resolved).length,sched=officialScheduleState(),next=plan[0]?.concept;
...
function lessonContent(c,pack,tab){const qs=V.QuestionQuality119?.forConcept(c.id)||[],detail=pack.detail||[],sections=uniqueSections(pack.deepSections||[]);
  if(tab==='detail'){const detailRows=sections.length?sections:uniqueTextRows(detail,pack.summary).map((body,i)=>({title:i===0?'정의 · 상세':'상세 정리',body,bullets:[]}));return `<div class="lesson detail-view">${studySchemaBlock(c,pack)}${detailRows.map(detailSection).join('')}${numberBlock(c,pack)}${visualBlocks(pack)}${hazmatBlock(c)}${specialCombustibleBlock(pack)}${calculationBlocks(c,pack)}${comparisonBlock(pack)}${trapBlock(pack)}</div>`}
  if(tab==='quiz'){
...
function tutorMessageHtml(m){
  const text=cleanTutorText(m?.text||'');
  return `<div class="row tutor-message ${m.role==='user'?'me':'assistant'}"><b>${m.role==='user'?'나':'119'}</b><p class="tutor-answer-text">${esc(text)}</p></div>`
}
...
function fallbackTutor(prompt,c,pack){
  ...
  if(compare&&(pack.compare||[]).length){
    rows.push('', '비교');
    for(const x of (pack.compare||[]).slice(0,6))rows.push(`${x[0]}: ${x[1]}`)
  }else{
...
async function sendTutor(){
  ...
  const detailed=wantsTutorDetail(prompt),compare=wantsTutorCompare(prompt);
  let out=fallbackTutor(prompt,current,pack);
  if(runtime.aiEngine||V.LocalAI?.ready){
    try{
      const privateText=await V.PrivateDocs.privateGrounding(current.title+' '+prompt,3).catch(()=>'');
      out=await V.LocalAI.chat([
        ...
        {role:'user',content:`[현재 개념]\n${current.id} ${current.title}\n[유형]\n${V.ConceptArchitecture119?.get?.(current.id)?.label||''}\n[요약]\n${pack.summary||''}\n[상세]\n${(pack.detail||[]).join('\n')}\n[시험필수]\n${(pack.must||[]).join('\n')}\n[비교]\n${(pack.compare||[]).map(x=>x.join(': ')).join('\n')}\n[함정]\n${(pack.traps||[]).join('\n')}\n[개인자료 - 현재 개념 질문과 관련된 경우에만 사용]\n${privateText||'없음'}\n[질문]\n${prompt}\n\n${compare?'비교 질문이면 먼저 차이를 짧게 나란히 제시하고 시험 함정을 덧붙여라.':''}${detailed?'교재형 상세 설명으로 확장하되 현재 개념 경계를 넘지 마라.':'결론부터 2~6개 핵심 항목으로 짧게 답하라.'}`}
      ],{temperature:.1,max_tokens:detailed?950:520})||out;
      out=cleanTutorText(out);runtime.aiEngine=V.LocalAI.engine||runtime.aiEngine
    }catch{}
  }
  state().chat[state().chat.length-1]={...assistant,text:out,targetConceptId:current.id};S.save();render()
}
