...
const runtime={more:false,account:false,bankConcept:'',bankFilter:'',bankIndex:0,calcGroup:'all',calcStage:'all',noteFilter:'all',noteQuery:'',studyQuizIndex:{},exam:null,examTimer:null,examDifficulty:'mid',examReportId:'',docs:[],docsOwner:'',docsLoading:false,docViewer:null,uploadStatus:'',aiEngine:null,aiStatus:'질문 가능',suggestions:[],suggestionsAdmin:false,suggestionsLoading:false,suggestionsOwner:'',questionAt:Date.now(),toast:'',authNotice:'',pendingAuthEmail:''};
...
const NAV=[['home','⌂','홈'],['study','▣','학습'],['tutor','✦','AI 질문'],['notes','▤','합격노트'],['bank','?','문제'],['exam','⏱','시험'],['wrong','!','오답'],['stats','▥','통계'],['resources','◎','자료'],['suggestions','✉','건의함'],['settings','⚙','설정']];
...
function detailSection(x){if(!x)return'';const rawTitle=String(x.title||'').trim();if(/개념\s*이해|개념\s*구조|읽는\s*순서|학습\s*순서|개념\s*구조와\s*읽는\s*순서/.test(rawTitle))return'';const body=studentStudyText(x.body),bullets=uniqueTextRows((x.bullets||[]).filter(Boolean),x.body).map(studentStudyText).filter(Boolean);return `<section class="detail-section"><div class="detail-copy"><h3>${esc(sectionTitle(rawTitle||'개념'))}</h3>${body?`<p>${esc(body)}</p>`:''}${bullets.length?`<ul class="detail-key-list">${bullets.map(v=>`<li class="detail-key"><span class="study-star">★</span><span class="study-key-text">${esc(v)}</span></li>`).join('')}</ul>`:''}</div></section>`}
...
function bank(){
  ...
  if(!qs.length)return shell('<div class="empty">선택한 공식군·단계에 준비된 문제가 없습니다.</div>','문제');
...
function buildWrongTraining(target,level){
  ...
  const pool=trainingPool('all').filter(q=>wrongIds.has(q.id)||weakConcepts.includes(q.conceptId)),direct=pool.filter(q=>wrongIds.has(q.id)),rest=pool.filter(q=>!wrongIds.has(q.id));
  const seed=sampleDistinct(direct,Math.min(target,direct.length)),used=new Set(seed.map(q=>q.id)),fill=sampleByDifficulty(rest.filter(q=>!used.has(q.id)),Math.max(0,target-seed.length),level),qs=sampleDistinct([...seed,...fill],Math.min(target,pool.length));
...
function startTraining(planKey){
  ...
  runtime.exam={id:crypto.randomUUID?crypto.randomUUID():'exam-'+Date.now(),mode:'training',trainingKey:plan.key,title:plan.label,difficulty:runtime.examDifficulty,qs:plan.qs,i:0,startedAt:Date.now(),answers:{},blueprint:{fire:plan.fire,ems:plan.ems,minutes:null,freshWindow:4,training:true,label:plan.label}};
...
function startExam(mode){const ready=V.examReadiness();if(mode==='real'&&!ready.ready)return toast('실전 문제은행을 준비 중입니다.');const qs=buildMock(mode,runtime.examDifficulty);if(qs.length!==65)return toast('65문항을 구성할 수 없습니다. 문제은행을 확인해주세요.');runtime.exam={id:crypto.randomUUID?crypto.randomUUID():'exam-'+Date.now(),mode,difficulty:runtime.examDifficulty,qs,i:0,startedAt:Date.now(),answers:{},blueprint:{fire:25,ems:40,minutes:65,freshWindow:4}};persistActiveExam();render();startExamTicker()}
function examRun(){const e=runtime.exam,q=e.qs[e.i],real=e.mode==='real',training=e.mode==='training',total=e.qs.length,left=real?Math.max(0,3900-Math.floor((Date.now()-e.startedAt)/1000)):null,answered=Object.keys(e.answers).length,label=real?'실전 모의고사':training?(e.title||'집중훈련'):'연습 모의고사';if(real&&left===0)setTimeout(()=>finishExam(true),0);return shell(`<div class="workspace single-pane exam-run-workspace"><header class="concept-head exam-head-clean"><span class="scope-label">${esc(label)} · 문제 ${e.i+1}/${total}</span><h2 ${real?'data-exam-timer':''}>${real?`${String(Math.floor(left/60)).padStart(2,'0')}:${String(left%60).padStart(2,'0')}`:training?`${answered}/${total} 답변`:'문제를 풀어보세요'}</h2><button class="btn small ghost" data-exam-abandon>시험 중단</button></header><div class="study-body exam-body"><div class="question-card exam-question-card"><div class="tiny muted exam-subject">${V.subjectLabel(q.subject)}</div><h2>${esc(q.q)}</h2><div class="choices">${q.choices.map((x,i)=>`<button class="choice ${e.answers[q.id]===i?'sel':''}" data-exam-answer="${i}">${i+1}. ${esc(x)}</button>`).join('')}</div></div></div><footer class="actionbar exam-footer"><button class="btn" data-exam-prev ${e.i===0?'disabled':''}>← 이전</button><span class="exam-answer-count"><b>${answered}/${total}</b><small>답변 완료</small></span><button class="btn primary" data-exam-next>${e.i===total-1?'시험 종료':'다음 →'}</button></footer></div>`,'시험')}
...
function docsListHtml(){return runtime.docs.length?runtime.docs.map(d=>{const quality=Number.isFinite(Number(d.extractionQuality))?Math.round(Number(d.extractionQuality)*100):null,review=(d.reviewPages||[]).length,ocr=(d.ocrPages||[]).length;return `<div class="row doc-row"><div><b>${esc(d.title)}</b><small>${d.pageCount}쪽 · ${d.extractionVersion?'정밀 추출':'기본 추출'}${quality!=null?` · 품질 ${quality}%`:''}${ocr?` · OCR ${ocr}쪽`:''}${review?` · ⚠ 확인 ${review}쪽`:''}</small></div><div class="toolbar"><button class="btn small" data-doc-open="${d.id}">내용 보기</button><button class="btn small primary" data-doc-pass="${d.id}">AI 합격노트 초안</button><button class="btn small ghost" data-doc-delete="${d.id}">삭제</button></div></div>`}).join(''):'<div class="empty" style="height:100px">등록된 자료가 없습니다.</div>'}
...
function noteFilterChips(){
  const all=state().notes||[],defs=[['all','전체'],['fire','소방'],['ems','구급'],['star','★핵심'],['question','문제'],['doc','내 자료'],['manual','직접메모']];
...
<section class="card"><div class="toolbar"><b>PDF / 사진 정밀 추출</b><span class="spacer"></span><label class="btn small primary">+ PDF / 사진 추가<input id="personalFile" type="file" accept="application/pdf,image/*,.txt" hidden></label></div><p class="tiny muted">PDF 텍스트층 → 저품질 페이지 OCR 2중 인식 → 신뢰도 판정 → 지원 기기에서는 로컬 AI 보정 순으로 처리합니다. 숫자·단위 보존 검사를 통과하지 못한 AI 교정은 자동 폐기합니다.</p><small class="upload-state" data-upload-status>${esc(runtime.uploadStatus||'')}</small></section>
...
function examReportView(rec){
  ...
  const missed=items.filter(x=>x.selected===undefined||x.selected!==x.q.a),scopeCounts=new Map(),familyCounts=new Map();
  ...
  const weak=[...scopeCounts.entries()].sort((a,b)=>b[1]-a[1]).slice(0,5);
  const skills=[...familyCounts.values()].sort((a,b)=>(a.correct/Math.max(1,a.total))-(b.correct/Math.max(1,b.total))||b.total-a.total);
  ...
  return `<div class="exam-report screen-scroll"><div class="toolbar exam-report-top"><button class="btn small ghost" data-report-close>← 최근 시험</button><span class="spacer"></span><span class="tag">${esc(modeLabel)}</span></div><section class="card exam-report-summary"><h2>${rec.score}점</h2><div class="exam-report-grid"><div><span>전체</span><b>${rec.correct}/${total}</b></div><div><span>소방학</span><b>${rec.fireCorrect}/${fireTotal}</b></div><div><span>응급처치</span><b>${rec.emsCorrect}/${emsTotal}</b></div><div><span>시간</span><b>${Math.floor((rec.elapsedSec||0)/60)}분 ${(rec.elapsedSec||0)%60}초</b></div></div><p class="tiny muted">미응답 ${rec.unanswered||0} · 오답 ${rec.incorrectQuestionIds?.length??missed.filter(x=>x.selected!==undefined).length}</p></section>${weak.length?`<section class="card"><b>취약 단원</b><div class="weak-list">${weak.map(([name,n])=>`<span class="weak-chip">${esc(name)} · ${n}</span>`).join('')}</div></section>`:''}${skills.length?`<section class="card"><b>문제 유형 분석</b><div class="weak-list">${skills.map(x=>`<button class="weak-chip skill-train-chip" data-skill-train="${esc(x.key)}">${esc(x.label)} · ${x.correct}/${x.total} · 훈련</button>`).join('')}</div><p class="tiny muted">학습 분석용 능력군입니다. 공식 시험의 출제비율을 의미하지 않습니다.</p></section>`:''}<section class="card exam-missed-section"><div class="toolbar"><b>오답·미응답 분석</b><span class="spacer"></span><span class="tiny muted">${missed.length}문항</span></div><div class="exam-miss-list">${missed.length?missed.map(row).join(''):'<div class="empty" style="height:120px">전 문항 정답입니다.</div>'}</div></section></div>`;
}
function stats(){
...
async function ensureSuggestions(force=false){
  const owner=V.Auth?.user?.id||'guest';
  if(owner==='guest'){runtime.suggestions=[];runtime.suggestionsAdmin=false;runtime.suggestionsOwner='guest';runtime.suggestionsLoading=false;return}
  if(runtime.suggestionsLoading||(!force&&runtime.suggestionsOwner===owner))return;
  runtime.suggestionsLoading=true;
  try{
    const [rows,admin]=await Promise.all([V.Suggestions?.list?.()||[],V.Suggestions?.isAdmin?.()||false]);
    runtime.suggestions=rows||[];runtime.suggestionsAdmin=!!admin;runtime.suggestionsOwner=owner
  }catch(err){runtime.suggestions=[];runtime.suggestionsAdmin=false;runtime.suggestionsOwner=owner;runtime.authNotice='건의함을 불러오지 못했습니다: '+String(err?.message||err).slice(0,70)}
  finally{runtime.suggestionsLoading=false;if(state().page==='suggestions')render()}
}
function suggestions(){
  ...
  <section class="card suggestion-form"><b>새 건의사항</b><div class="form-grid" style="margin-top:10px"><label>분류<select id="suggestCategory" class="select">${(V.Suggestions?.CATEGORY||['개선','건의','오류','콘텐츠','기타']).map(x=>`<option>${esc(x)}</option>`).join('')}</select></label><label>제목<input id="suggestTitle" class="input" maxlength="120" placeholder="무엇을 개선하면 좋을까요?"></label></div><textarea id="suggestBody" class="textarea" maxlength="5000" placeholder="문제 화면, 원하는 개선점, 재현 방법 등을 자세히 적어주세요." style="margin-top:8px"></textarea><label class="suggest-anon"><input id="suggestAnonymous" type="checkbox" checked> 익명으로 보내기</label><button class="btn primary block" data-suggest-submit>건의사항 보내기</button></section>
  <section class="card"><div class="toolbar"><b>${admin?'전체 건의사항':'내 건의사항'}</b><span class="spacer"></span><span class="tiny muted">${runtime.suggestionsLoading?'불러오는 중':runtime.suggestions.length+'건'}</span></div><div class="suggestion-list">${runtime.suggestionsLoading?'<div class="empty" style="height:100px">불러오는 중…</div>':suggestionRowsHtml()}</div></section></div>`,'건의함')
}
...
if('suggestSubmit'in b.dataset){const category=$('#suggestCategory')?.value||'개선',title=$('#suggestTitle')?.value.trim()||'',body=$('#suggestBody')?.value.trim()||'',anonymous=$('#suggestAnonymous')?.checked!==false;try{await V.Suggestions.create({category,title,body,anonymous});runtime.suggestionsOwner='';toast('건의사항이 접수되었습니다.');await ensureSuggestions(true);return}catch(err){return toast('건의 접수 실패 · '+String(err?.message||err).slice(0,60))}}
...
if(b.dataset.suggestDelete){if(!confirm('이 건의사항을 삭제할까요?'))return;try{await V.Suggestions.remove(b.dataset.suggestDelete);runtime.suggestionsOwner='';toast('건의사항을 삭제했습니다.');await ensureSuggestions(true);return}catch(err){return toast('삭제 실패 · '+String(err?.message||err).slice(0,60))}}
...
document.addEventListener('change',async e=>{if(e.target.id==='personalFile'){const f=e.target.files?.[0];if(!f)return;runtime.uploadStatus='정밀 분석 준비 중';const update=(t)=>{runtime.uploadStatus=t;const el=document.querySelector('[data-upload-status]');if(el)el.textContent=t};update('정밀 분석 중…');try{const result=await V.PrivateDocs.ingest(f,{kind:'personal',keepOriginal:true,aiAssist:'auto',onProgress:(i,n,mode,detail)=>{const label=mode==='ocr'?'OCR 정밀인식':mode==='ocr+ai'||mode==='ai'?'로컬 AI 보정':mode==='native-preferred'?'텍스트층 검증':'텍스트 추출';update(detail||`${label} · ${i}/${n}쪽`)}});runtime.docsOwner='';await ensureDocs();const review=result.reviewPages?.length||0;update(review?`분석 완료 · 원문 확인 권장 ${review}쪽`:`분석 완료 · 품질 ${Math.round((result.extractionQuality||0)*100)}%`);toast(review?`자료 분석 완료 · ${review}쪽 확인 필요`:'자료 정밀 분석 완료')}catch(err){update('분석 실패 · 오류 내용을 확인해주세요');toast('자료 처리 실패: '+String(err.message||err).slice(0,70))}}if(e.target.id==='importBackup'){const f=e.target.files?.[0];if(!f)return;try{S.import(await f.text());toast('복원 완료');render()}catch{toast('복원 실패')}}});
window.addEventListener('aitutor-auth-change',e=>{
  runtime.docsOwner='';
  runtime.suggestionsOwner='';runtime.suggestions=[];runtime.suggestionsAdmin=false;
...
V.App={render,go,chooseConcept,runtime};render();if(restoredActiveExam)startExamTicker();
})();
