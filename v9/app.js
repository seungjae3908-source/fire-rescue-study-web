...
function visibleTutorChat(){return[...document.querySelectorAll('.study-ai-chat')].find(e=>e.offsetParent!==null)||null}
function tutorScrollSnapshot(){const x=visibleTutorChat();if(!x)return null;const b=x.closest('.study-body'),bottom=e=>!e||e.scrollHeight<=e.clientHeight+6||Math.abs(e.scrollHeight-e.clientHeight-e.scrollTop)<=24;return{conceptId:currentConcept()?.id||'',chatTop:x.scrollTop,bodyTop:b?.scrollTop||0,pinned:bottom(x)&&bottom(b)}}
function restoreTutorScroll(snap,force=false){requestAnimationFrame(()=>requestAnimationFrame(()=>{const x=visibleTutorChat();if(!x)return;const b=x.closest('.study-body'),same=!snap||snap.conceptId===(currentConcept()?.id||'');if(force||!snap||!same||snap.pinned){x.scrollTop=x.scrollHeight;if(b)b.scrollTop=b.scrollHeight}else{x.scrollTop=Math.min(snap.chatTop,Math.max(0,x.scrollHeight-x.clientHeight));if(b)b.scrollTop=Math.min(snap.bodyTop,Math.max(0,b.scrollHeight-b.clientHeight))}if(force)runtime.tutorForceLatest=false}))}
function render(){const tutor=state().page==='study'&&state().studyTab==='ai',snap=tutor?tutorScrollSnapshot():null,force=!!runtime.tutorForceLatest;document.querySelector('#app').innerHTML=view();if(tutor)restoreTutorScroll(snap,force)}
