...
function detailSection(x,index=0){if(!x)return'';const rawTitle=String(x.title||'').trim();if(/개념\s*이해|개념\s*구조|읽는\s*순서|학습\s*순서|개념\s*구조와\s*읽는\s*순서/.test(rawTitle))return'';const body=studentStudyText(x.body),bullets=uniqueTextRows((x.bullets||[]).filter(Boolean),x.body).map(studentStudyText).filter(Boolean);return `<section class="detail-section" id="detail-section-${index}"><div class="detail-copy"><h3>${esc(sectionTitle(rawTitle||'개념'))}</h3>${body?`<p>${esc(body)}</p>`:''}${bullets.length?`<ul class="detail-key-list">${bullets.map(v=>`<li class="detail-key"><span class="study-star">★</span><span class="study-key-text">${esc(v)}</span></li>`).join('')}</ul>`:''}</div></section>`}
function detailToc(rows=[]){
  ...
  return items.length>2?`<nav class="detail-toc" aria-label="상세 목차"><b>상세 목차</b><div>${items.map(x=>`<button class="btn small ghost" data-book-jump="detail-section-${x.i}">${esc(x.title)}</button>`).join('')}</div></nav>`:''
}
