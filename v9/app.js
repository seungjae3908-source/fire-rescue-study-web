...
function examReportView(rec){
  ...
  return `<div class="exam-report screen-scroll"><div class="toolbar exam-report-top"><button class="btn small ghost" data-report-close>← 최근 시험</button><span class="spacer"></span><span class="tag">${esc(modeLabel)}</span></div><section class="card exam-report-summary"><h2>${rec.score}점</h2><div class="exam-report-grid"><div><span>전체</span><b>${rec.correct}/${total}</b></div><div><span>소방학</span><b>${rec.fireCorrect}/${fireTotal}</b></div><div><span>응급처치</span><b>${rec.emsCorrect}/${emsTotal}</b></div><div><span>시간</span><b>${Math.floor((rec.elapsedSec||0)/60)}분 ${(rec.elapsedSec||0)%60}초</b></div></div><p class="tiny muted">미응답 ${rec.unanswered||0} · 오답 ${rec.incorrectQuestionIds?.length??missed.filter(x=>x.selected!==undefined).length}</p></section>${weak.length?`<section class="card"><div class="toolbar"><b>취약 단원</b><span class="spacer"></span><small class="tiny muted">눌러서 바로 복습</small></div><div class="weak-list">${weak.map(x=>`<button class="weak-chip skill-train-chip weak-review-chip" data-concept="${esc(x.conceptId)}" data-weak-review="${esc(x.scopeId)}">${esc(x.title)} · 오답 ${x.count} · 복습</button>`).join('')}</div></section>`:''}${skills.length?`<section class="card"><b>문제 유형 분석</b><div class="weak-list">${skills.map(x=>`<button class="weak-chip skill-train-chip" data-skill-train="${esc(x.key)}">${esc(x.label)} · ${x.correct}/${x.total} · 훈련</button>`).join('')}</div><p class="tiny muted">학습 분석용 능력군입니다. 공식 시험의 출제비율을 의미하지 않습니다.</p></section>`:''}<section class="card exam-missed-section"><div class="toolbar"><b>오답·미응답 분석</b><span class="spacer"></span><span class="tiny muted">${missed.length}문항</span></div><div class="exam-miss-list">${missed.length?missed.map(row).join(''):'<div class="empty" style="height:120px">전 문항 정답입니다.</div>'}</div></section></div>`;
}
function stats(){
