...
function studyHighlight(v,pack){const t=String(v||''),a=coreHighlightTerms(pack).filter(x=>t.includes(x)),n=t.match(/\d+(?:\.\d+)?(?:\s*(?:~|–|-)\s*\d+(?:\.\d+)?)?\s*(?:초|분|시간|cm|mm|m|kg|L|%|J\/kg)?/g)||[],terms=[...new Set([...a,...n].filter(Boolean))].sort((x,y)=>y.length-x.length);if(!terms.length)return esc(t);const re=new RegExp('('+terms.map(x=>x.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')).join('|')+')','g');return t.split(re).map((x,i)=>i%2?`<u class="study-key-underline ${/\d/.test(x)?'study-key-number':'study-key-term'}">${esc(x)}</u>`:esc(x)).join('')}
