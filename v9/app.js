...
function examRun(){
const e=runtime.exam,q=e.qs[e.i],real=e.mode==='real',training=e.mode==='training',total=e.qs.length,left=real?Math.max(0,3900-Math.floor((Date.now()-e.startedAt)/1000)):null,answered=Object.keys(e.answers).length,remaining=Math.max(0,total-answered),label=real?'실전 모의고사':training?(e.title||'집중훈련'):'연습 모의고사',pct=Math.round(answered/Math.max(1,total)*100);
