...
return `<article class="book-mobile"><nav class="book-jumpbar">${STUDY_TABS.map(([k,l])=>`<button class="${tab===k?'on':''}" data-study-tab="${k}">${l}</button>`).join('')}</nav><section class="book-section">${lessonContent(c,pack,tab)}</section><footer class="mobile-study-nav" aria-label="개념 이동"><button class="btn ghost" data-study-prev ${nb.prev?'':'disabled'} aria-label="이전 개념">← 이전</button><button class="btn ghost" data-outline>목차</button><button class="btn primary" data-study-next ${nb.next?'':'disabled'} aria-label="다음 개념">다음 →</button></footer></article>`
}
...
if('resourcePdfJump'in b.dataset){const root=document.querySelector('#resourcePdf'),key=root?.dataset.docKey||'',raw=root?.querySelector('[data-resource-pdf-jump-input]')?.value||'',page=requestedPdfPage(key,raw);if(!page)return toast('이동할 쪽 번호를 입력하세요.');delete root.dataset.searchQuery;await renderResourcePdf(key,page);return}
if('resourcePdfSearch'in b.dataset){const root=document.querySelector('#resourcePdf'),key=root?.dataset.docKey||'',input=root?.querySelector('[data-resource-pdf-search-input]'),q=input?.value.trim()||'';if(q.length<2)return toast('검색어를 2글자 이상 입력하세요.');const badge=root.querySelector('[data-resource-page-label]');if(badge)badge.textContent='원문 검색 중…';const hit=await locatePdfText(key,q);if(hit.error)return toast(hit.error==='SEARCH_NOT_FOUND'?'검색 결과가 없습니다.':'원문 검색에 실패했습니다.');root.dataset.searchQuery=q;await renderResourcePdf(key,hit.page);return}
...
if(b.dataset.passCore){try{const r=await V.PassNote.toggleConceptCore(b.dataset.passCore);toast(r.saved?'합격노트에 추가됨':'합격노트에서 삭제됨');return render()}catch(err){return toast('합격노트 저장 실패 · '+String(err?.message||err).slice(0,40))}}
